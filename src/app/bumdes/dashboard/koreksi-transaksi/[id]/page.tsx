...
import { postJournalCorrectionAction } from "../actions";
...
export default async function BumdesKoreksiTransaksiDetailPage({
  params,
}: {
  ...
  const isReadyToPost =
    asText(correction.correction_status) === "approved" &&
    asText(correction.flow_status) === "READY_TO_POST";

  const canPost = context.role === "admin_bumdes" && isReadyToPost;

  return (
    <div className="space-y-5">
      <div>
        <Link
          href="/bumdes/dashboard/koreksi-transaksi"
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-wide text-emerald-700">
              Admin BUMDes / Detail Koreksi Transaksi
            </p>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
              Posting koreksi akan membuat jurnal pembalik otomatis, memposting
              jurnal pengganti, dan mengubah transaksi lama menjadi reversed.
            </p>
          </div>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-slate-500">Disetujui Oleh</p>
              <p className="mt-1 font-bold text-slate-950">
                {asText(correction.approved_by_name)}
              </p>
              <p className="text-xs text-slate-500">
                {formatDateTime(correction.approved_at)}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-slate-500">Transaksi Lama</p>
              <p className="mt-1 font-bold text-slate-950">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-slate-500">Transaksi Pengganti</p>
              <p className="mt-1 font-bold text-slate-950">
      ...
      <Card>
        <CardHeader
          ...
          description="Alasan yang telah direview dan disetujui Pengawas."
      ...
      <Card>
        <CardHeader
          ...
          description="Transaksi lama, pembatalan otomatis, dan transaksi pengganti untuk audit sebelum posting."
          action={<Badge variant="neutral">{lineRows.length} baris</Badge>}
        />

        <DataTable
          columns={["Jenis", "Nomor Jurnal", "Akun", "Keterangan", "Debit", "Kredit"]}
          emptyText="Belum ada detail baris transaksi."
        >
          {lineRows.length > 0
            ? lineRows.map((line, index) => (
                <tr
                  key={`${asText(line.journal_role)}-${asText(line.journal_line_id)}-${index}`}
      ...
      <section className="grid gap-5 xl:grid-cols-[1fr_0.8fr]">
        <Card>
          <CardHeader
            ...
            description="Jejak proses koreksi dari draft sampai posting."
                ...
                >
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <div className="text-left text-xs text-slate-500 sm:text-right">
                      <p className="font-semibold">{asText(item.actor_name)}</p>
        ...
        <Card>
          <CardHeader
            title="Posting Koreksi"
            description="Posting hanya tersedia setelah koreksi disetujui Pengawas."
            action={
              isReadyToPost ? (
                <Badge variant="warning">Siap Posting</Badge>
              ) : (
                <StatusBadge status={correction.correction_status} />
              )
            }
          ...
          <div className="space-y-4 px-5 pb-5">
            {canPost ? (
              <form action={postJournalCorrectionAction}>
                ...
                <p className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-800">
                  Setelah diposting, transaksi lama menjadi reversed, jurnal
                  pembalik dibuat otomatis, dan jurnal pengganti menjadi posted.
                </p>
              </form>
            ) : (
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm leading-6 text-slate-600">
                {context.role !== "admin_bumdes" ? (
                  <>
                    Role saat ini adalah{" "}
                    <span className="font-bold text-slate-950">{context.role}</span>.
                    Tombol posting hanya tersedia untuk Admin BUMDes.
                  </>
                ) : (
                  <>
                    Koreksi ini belum berada pada status siap posting. Status saat
                    ini:{" "}
