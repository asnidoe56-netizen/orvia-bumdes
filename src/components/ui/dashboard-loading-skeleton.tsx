      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div
          className={joinClassNames(
            "h-5 rounded-xl bg-slate-200",
            titleWidthClassName
          )}
        />
        <div className="mt-3 h-4 w-72 max-w-full rounded-xl bg-slate-100" />
      </section>
        ...
        <section className="grid gap-4 md:grid-cols-3">
          {Array.from({ length: statCount }).map((_, index) => (
            ...
            >
              ...
              <div className="mt-4 h-7 w-20 rounded-xl bg-slate-200" />
              <div className="mt-3 h-3 w-32 rounded-xl bg-slate-100" />
            </div>
          ))}
        </section>
      ) : null}
